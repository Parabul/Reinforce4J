...
public class PlayoutSimulator<T extends GameState> {

  private static final int MAX_MOVES = 500;

  private final EnumMap<Player, Strategy<T>> strategies;
  private final GameService<T> service;

  public PlayoutSimulator(
      Strategy<T> playerOneStrategy, Strategy<T> playerTwoStrategy, final GameService service) {
    this.strategies =
        new EnumMap<>(
